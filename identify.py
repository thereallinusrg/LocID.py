...
def tri_split(snx, sny, lvl, lst, gdf, poly):
    ...
    # Check if we need to handle a turned triangle (each 0 facet turns everything inside)
    if lst.count(0) % 2 != 0:
    ...
    if (lvl > 1):   # recursive call
        lvl = lvl - 1
        snx2, sny2 = tri_split_helper(tri2)
        gdf2 = tri_split(snx2, sny2, lvl, lst + [2], gdf, poly)
        snx0, sny0 = tri_split_helper(tri0)
        gdf0 = tri_split(snx0, sny0, lvl, lst + [0], gdf, poly)
        snx3, sny3 = tri_split_helper(tri3)
        gdf3 = tri_split(snx3, sny3, lvl, lst + [3], gdf, poly)
        snx1, sny1 = tri_split_helper(tri1)
        gdf1 = tri_split(snx1, sny1, lvl, lst + [1], gdf, poly)
        gdf = pd.concat([gdf2, gdf0, gdf3, gdf1], ignore_index=True)

    elif (lvl > 0):
        g2 = gpd.GeoDataFrame(pd.DataFrame(
            {'qloc': [lst+[2]]}, index=[0]), geometry=[Polygon(tri2)])
        g0 = gpd.GeoDataFrame(pd.DataFrame(
            {'qloc': [lst+[0]]}, index=[0]), geometry=[Polygon(tri0)])
        g3 = gpd.GeoDataFrame(pd.DataFrame(
            {'qloc': [lst+[3]]}, index=[0]), geometry=[Polygon(tri3)])
        g1 = gpd.GeoDataFrame(pd.DataFrame(
            {'qloc': [lst+[1]]}, index=[0]), geometry=[Polygon(tri1)])
...
def stack2gdf(stacks, SeT_lvl, max_lvl):
    ...
    gdf = gpd.GeoDataFrame(data={'n': nlst, 'lvl': lvlst, 'node_x': xlst,
                           'node_y': ylst, 'QID': qlst, 'xnode': xnlst, 'ynode': ynlst}, geometry=glst)
...
def create_missing_facets(gdf, polygon):
    ...
    covering_gdf = gdf_f[gdf_f['geometry'].intersects(
        msng_pc['geometry'].iloc[0])]
    ...
    while len(msng_pc) > 0:
        level = level - 1
        gdf_f = gdf[gdf['lvl'] == level]
        covering_gdf = gdf_f[gdf_f['geometry'].covers(
            msng_pc['geometry'].iloc[0])]
        grid = pd.concat([grid, covering_gdf], ignore_index=True)
    ...
    for index, row in grid_ex.iterrows():
        ...
        out_gdf['qloc'] = out_gdf.apply(
            lambda row: prepend_qloc(row, qloc_base), axis=1)
...
def map_poly(polygon, tol, lvl, bytes_out=False):
    ...
    for cord in pzxy:  # This is intentionally a loop, as g2q works with a memory stack, not calculating everything again
        qid, QL, lastMatch, stack = q.g2q(
            cord[1], cord[0], tol, qid_only=False, bytes_out=bytes_out)

        if lastMatch == max_lvl:
            continue  # Skipping duplicate encodings (in the same facet)
        elif lastMatch > max_lvl:
            max_lvl = deepcopy(lastMatch)

        # deepcopy is important, due to the global stack --> to not modify to global stack
        stacks.append(deepcopy(stack))
        pqxy.append(qid)

        if SeT_lvl > lastMatch:
            SeT_lvl = lastMatch
    ...
    if SeT_lvl > lvl:
        raise ValueError(
            f"The minimum encoding level (lvl) must at least be as high as the SeT level, which is {SeT_lvl}")
...
def encode_polygon(polygon, tol, lvl, bytes_out=False):
    ...
    '''
    qid_lst, SeT_qid = map_poly(
        polygon, tol, lvl, bytes_out=False)  # Polygon Mapping is only supported with bytes_out = False atm
...
def identify(geo_object, tol=1e-06, lvl=20):
    ...
    # TODO: Implement line encoding
        ...
        raise NotImplementedError(
            "MultiLineString type is not implemented yet")
    else:
